    ...
    )
{
    ...
    // first initialize the RunningThreadsMinPriority value to the highest priority value
...
)
{
    ...
    // ensure we aren't in an interrupt context
    ASSERT(INTR_OFF == CpuIntrGetState());
    ASSERT(NULL != pCpu);

    // trigger thread yield after the interrupt is handled
    pCpu->ThreadData.YieldOnInterruptReturn = TRUE;
    ...
    )
{
    ...
    if (newPrio > minPrio) {
        // preempt the smaller-priority running thread
